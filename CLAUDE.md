...
| `GET /api/levels?symbol=&tf=` | niveaux en chiffres à recopier dans MT5 (déclenchement manuel, hors boucle de trading) |
...
- **Dimensionnement du SL testable** — `SL_ATR_MULT` (1.8), `SL_ATR_MULT_HIGH` (2.0), `SL_MIN_ATR_MULT` (1.2) exposés en overrides walk-forward. Motivation : le MFE médian s'écrase quand l'horloge ralentit (1.4–1.7R en M5 contre 0.75–0.92R en H1) — le SL s'élargit avec l'ATR mais le mouvement qui suit ne suit pas proportionnellement.
- **`_diag_mae_distribution()` ne produit AUCUNE espérance, volontairement.** `MAE ≥ f` équivaut exactement à « le prix a atteint f×R contre nous », donc le taux de touche d'un SL candidat est exact. Mais **MAE et MFE ne sont pas ordonnés** : on ignore si un trade tué par un SL resserré avait déjà encaissé TP1. Toute formule d'espérance bâtie dessus serait fausse — c'est l'erreur commise sur `_diag_mfe_distribution`, dont le `−1R` pour tout trade n'atteignant pas TP2 ignorait les sorties intermédiaires (timeout, early exit) et **inversait la conclusion** : elle annonçait TP2=1.0R 35× meilleur que 1.8R, le walk-forward a donné PF 0.93 contre 1.03. Ce tableau sert à choisir les valeurs à tester, jamais à prédire leur résultat.
- **`data_provider.market_symbol()` lève sur un symbole inconnu** au lieu du repli muet `MARKET_SYMBOLS.get(symbol, <XAUUSD>)`. Un walk-forward lancé sur `"ES"` (absent de `MARKET_SYMBOLS`, l'ES passant par le proxy SPY de `pretrain_es`) chargeait des données XAU/USD et les présentait comme de l'ES, sans erreur ni avertissement.
- **Panneau « Niveaux à tracer — MT5 » (`levels.py`, `GET /api/levels`)** — aide au tracé manuel, hors boucle de trading : aucun signal, aucun score, aucune espérance. Déclenché par un bouton, jamais rafraîchi seul. Il n'utilise **pas** `strategy.find_order_blocks` (le détecteur du graphique), qui accepte toute bougie rouge suivie d'une verte avec 0.5×ATR d'impulsion **sans vérifier la mitigation** — d'où les 5-7 zones empilées dont la plupart ont déjà été traversées ; il appelle `strategy_ict._find_order_blocks` avec `require_bos=True` (nouveau paramètre facultatif, `None` = suivre `OB_REQUIRE_BOS`, donc chemin live inchangé) et plafonne à 2 zones par côté, les plus proches du prix, en écartant celles que le prix a déjà dépassées. Le compte de touches d'un S/R est un **nombre de contacts distincts**, pas de bougies : à 0.5×ATR de tolérance, un range affichait « 28 touches » pour un niveau visité une fois. Un niveau à moins d'une tolérance du prix est écarté (on est dessus). Le canal est marqué `fiable: False` sous 2 contacts par bord — deux parallèles calées sur les extrêmes encadrent le prix **par construction**, seules les touches disent s'il existe (même convention que les niveaux censurés de la distribution du MFE).
- **Fenêtres du panneau « Niveaux » volontairement découplées** — `SR_LOOKBACK` (400) ≫ `CHANNEL_LOOKBACK` (120). Le canal décrit le mouvement en cours ; un support reste un support à 300 bougies. Tant que les deux partageaient 120 bougies, un prix atteignant l'extrême de la fenêtre n'avait plus **aucun** niveau d'un côté par construction — le panneau se taisait exactement au moment où l'on cherche le niveau suivant (constaté en live : or à 4380, « aucun support », prix à 0.5 % de la hauteur du canal). `OB_LOOKBACK_BARS` passe de 60 à 150 pour la même raison. Chaque côté vide renvoie désormais **son motif** (`sr_notes`, `ob_notes`) : « le prix est au plus bas des N bougies » est une information, un blanc se lit comme une panne, et « aucune zone détectée » vs « toutes déjà dépassées par le prix » sont deux situations opposées.
- **Fibonacci du panneau « Niveaux » (`levels.fibonacci`)** — le seul vrai choix d'un Fib est le segment ; les ratios ne se discutent pas et les prix en découlent. Il est pris **mécaniquement** (plus haut et plus bas de `FIB_LOOKBACK`, dans l'ordre chronologique) pour être reproductible à l'identique dans MT5, et le segment retenu est renvoyé en clair (dates + prix des deux extrémités) pour qu'on voie immédiatement s'il faut ignorer le bloc. Une seule formule, `prix(r) = fin − r × (fin − départ)` : `r=1` retombe sur l'origine, `r>1` la dépasse — exactement les niveaux > 100 % de l'outil MT5, donc pas de seconde convention « extension » à côté. Refusé sous `FIB_MIN_LEG_ATR` (1.5×ATR) : en range les sept niveaux seraient séparés par moins d'une mèche. **Aucune valeur prédictive n'est affirmée** — rien dans ce dépôt n'a jamais validé Fibonacci sur l'or. Le seul chiffre mesuré est le nombre de contacts distincts **depuis la fin de la jambe** (les compter pendant la jambe ferait passer un niveau traversé une fois en montant pour un niveau respecté) ; un test interdit toute clé d'espérance ou de probabilité, pour la raison qui a fait retirer celle du MFE.
