...
MAX_TRADE_MINUTES = 75
TREND_BIAS_DISTANCE   = 0.3      # bloque SHORT si price > EMA200+0.3×ATR (et LONG inverse)
...
RSI_H1_SHORT_MAX      = 60.0    # RSI H1 maximal pour SHORT — H1 pas trop bullish
...
def evaluate_eurusd(
    ...
    if bias == "LONG":
        ...
        sl = max(sl, entry - SL_ATR_MULT * atr_val)  # plafond : jamais plus large que SL_ATR_MULT×ATR
        direction = "long"
    else:
...
def snapshot(m5: pd.DataFrame, m15: pd.DataFrame, h1: pd.DataFrame,
    ...
    return {
        ...
        "atr_m5": _f(cur5, "atr", 5),
        "atr_avg": round(float(m5["atr"].tail(50).mean()), 5) if len(m5) else None,
        "adx_h1": _f(cur_h1, "adx", 1),
