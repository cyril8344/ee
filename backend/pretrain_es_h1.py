...
# --------------------------------------------------------------------------- #
def _load_es_h1_data(start: str, end: str) -> pd.DataFrame:
    """Télécharge ES=F en bougies 60 minutes depuis yfinance, fallback synthétique."""
    try:
        tk = yf.Ticker("ES=F")
        ...
        pass
    return _synthetic_es_h1(start, end)


def _synthetic_es_h1(start: str, end: str) -> pd.DataFrame:
    """Données synthétiques ES H1 pour les tests offline / fallback."""
...
def run_pretrain_es_h1(
    ...
    try:
        ...
        raw = _load_es_h1_data(start, end)
